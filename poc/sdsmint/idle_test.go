...
// difference between this PoC's measured behaviour and the new one. Phase 6's
...
// TestNullMinterDoesNotForget documents the deliberate asymmetry. The null
// minter hands out a fixed pre-signed pool, so there is nothing per-name to
// release -- and phase 6, which runs against it, therefore measures Envoy's
// reclamation alone rather than a mixture of the two.
func TestNullMinterDoesNotForget(t *testing.T) {
	nm := testNullMinter(t, NullMinterOptions{Host: "*.example", TTL: time.Hour})
	if _, ok := Minter(nm).(Forgetter); ok {
		t.Error("NullMinter implements Forgetter; phase 6 would no longer be measuring Envoy alone")
	}
}
