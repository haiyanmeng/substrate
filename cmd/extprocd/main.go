...
func run(ctx context.Context) error {
	...
	// Everything below, and everything in cmd/extprocd/internal/inner, logs
	// through the default logger rather than one passed down by hand.
	...
	extprocv3.RegisterExternalProcessorServer(grpcServer, inner.NewServer())

	slog.InfoContext(ctx, "extprocd listening", slog.String("address", lis.Addr().String()))

	ctx, stop := signal.NotifyContext(ctx, syscall.SIGINT, syscall.SIGTERM)
	defer stop()
	go func() {
		<-ctx.Done()
		// Not InfoContext: ctx is already cancelled here, and a handler that
		// honors cancellation would drop the one line that says why the
