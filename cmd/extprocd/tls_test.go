...
func TestGatewayTLSConfig(t *testing.T) {
	manifest, err := os.ReadFile("../../manifests/ate-install/atenet-egress-extproc.yaml")
	if err != nil {
		t.Fatal(err)
	}
...
// which is the shape credbundle.Parse expects.
...
func parseBundle(t *testing.T, bundle []byte) tls.Certificate {
	...
	cert, err := credbundle.Parse(path)
	if err != nil {
		t.Fatal(err)
	}
	return *cert
...
// writes counts every file this test package has written, so each one can be
// given a distinct mtime. The loaders cache on mtime and size, and a rotation
// written within one filesystem timestamp tick would otherwise be invisible to
// them -- a flake that would look like the rotation support not working.
var writes int
