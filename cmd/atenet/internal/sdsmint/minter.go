...
// a memory hint, not part of issuance: a minter with nothing to release -- one
// serving from a fixed pool, or one that does not cache at all -- is perfectly
// correct without it. Callers must type-assert.
//
...
func (m *minter) GetCertificate(ctx context.Context, host string) (*MintedCert, error) {
	...
	// that holding the map lock across it would serialize every handshake.
...
// silently authorize the CA for every depth of subdomain.
...
// AllowAny builds a validate function accepting every syntactically plausible
// hostname, at any subdomain depth and in any TLD.
//
// This is not a wide allowlist, it is the absence of one: the operator is
// saying the destination policy lives somewhere else entirely (the gateway's
// route matching, an ext_proc, the network the gateway sits on) and that
// sdsmint should mint for whatever SNI reaches it. Pair it only with a CA
// whose reach is bounded some other way -- a name-constrained root still
// cannot sign outside its constraint no matter what this function returns,
// and with an unconstrained root this is the last thing that would have said
// no.
//
// checkHostSyntax still applies. Its rejections are not policy but hygiene:
// a name with a wildcard, a path separator or an embedded newline in it is a
// caller trying to smuggle something into a certificate, not a destination.
func AllowAny() func(string) error {
	return checkHostSyntax
}
