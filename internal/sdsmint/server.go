...
type Server struct {
	...
	log     *slog.Logger
	metrics *Metrics
	// rotate enables the proactive re-mint push. Off by default: it is a
	// sustained cost proportional to the live set, not a free improvement, so
...
//
// Note on failure signaling: the intuitive design is to NACK a name that fails
...
func (s *Server) DeltaSecrets(stream secretservice.SecretDiscoveryService_DeltaSecretsServer) error {
	...
	// Sends are funneled through one goroutine because gRPC forbids
