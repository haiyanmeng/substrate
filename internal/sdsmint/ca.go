...
type CA struct {
	...
	// renewMu serializes renewal so a burst of concurrent Signs past the
...
func keyMatchesCert(cert *x509.Certificate, key crypto.Signer) bool {
	...
	if !ok {
		// An unrecognized key type cannot be compared. Do not block on it.
		return true
	}
...
//
// It returns a localca.CA so the caller can serialize it into a pool file in
