...
// Validate reports whether the CA is well formed enough to sign with. Nothing
// on the parse path checks this today: Unmarshal reads the signing key and the
// root certificate independently, so a pool assembled by hand round-trips
// cleanly and only fails at the first handshake. GenerateCA, by contrast,
// produces a CA that satisfies all of this by construction.
//
// This is about the CA alone. What a particular consumer can serve -- whether
// it has a code path for intermediates, say -- is that consumer's rule to
// enforce on top of this one.
...
//
// A key type that cannot be compared passes, which makes this a "no known
// mismatch" check rather than proof of a match. Refusing to load a CA because
// its algorithm is unfamiliar would be the worse failure of the two.
