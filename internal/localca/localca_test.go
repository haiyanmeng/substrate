...
func TestExternalSignerCanIssueButCannotBeMarshalled(t *testing.T) {
	...
	// Serialising does not, and must say why.
	_, err = Marshal(&Pool{CAs: []*CA{ca}})
	if err == nil {
		t.Fatal("Marshal serialised a signer with no exportable key material")
	}
...
func TestGenerateCAMaxPathLen(t *testing.T) {
	...
	// Historical behaviour: no path length stated at all. A parsed certificate
