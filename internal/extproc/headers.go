...
// tunnelInternalHeaders must never reach the real destination.
//
// The three X-Ate-* names were how an actor used to identify itself. They are
// no longer read anywhere -- identity comes from the client certificate, see
// identity.go -- but an actor can still put them on a request inside the
// tunnel, and forwarding them would leak substrate-internal naming to a third
// party. They are written out here rather than referenced from internal/atunnel
// because atunnel no longer defines them; nothing sends these, so there is
// nothing left to drift from.
var tunnelInternalHeaders = []string{
	"x-ate-atespace",
	"x-ate-actor-name",
	"x-ate-actor-version",
	EgressModeHeader,
...
type requestInfo struct {
	...
	method    string
	authority string
	// actor is the identity Envoy authenticated in the TLS handshake, read back
	// out of the peer certificate. Unlike the X-Ate-* headers this replaced, it
	// is not a claim the caller can make about itself. Zero when the peer
	// presented a valid certificate that carries no ActorIdentity.
	actor ActorKey
	// actorUID is logged, not matched on. Policy is keyed on (atespace, name),
	// which outlives any one activation; the UID identifies the activation and
	// is what makes an audit line resolvable back to a specific Pod.
	actorUID string
	// identityErr is why the peer certificate yielded no actor. Non-nil means
	// malformed input or a failed validation, which is distinct from a
	// well-formed certificate that simply is not an actor's.
	identityErr error
}
...
func newRequestInfo(hs []*corev3.HeaderValue) *requestInfo {
	...
	info.actor, info.actorUID, info.identityErr = actorFromXFCC(m[XFCCHeader])
	return info
}
