...
const (
	...
	actorCredentialPath   = "/run/actor-identity/credential-bundle.pem"
...
//
// It is shared rather than per-test because resuming an actor costs more than
// everything else the suite does put together, and because nothing here is
// actor-specific: the tests need an actor that exists and is running, not a
// fresh one. Teardown is registered with the suite rather than with the test
// that happened to trigger creation, which would suspend it out from under the
// tests that ran after it.
//
// What the actor's workload does is irrelevant -- it is never contacted. All
// that matters is that the ate API reports STATUS_RUNNING for the UID the
// certificate carries, because that is exactly what the gateway's ext_proc
// checks (cmd/atenet/internal/router/egress/egress.go, validateActor).
...
func createLiveActor() (*probeActor, error) {
	...
	// Boot from the template's spec instead of restoring its golden snapshot.
	// The suite needs an actor the control plane calls RUNNING, and nothing
	// about the guest's state; booting keeps that from depending on a golden
	// snapshot that some other suite's fixture owns and that a schema change to
	// the snapshot record can make unreadable long after it was written.
	...
	// Poll rather than trust the resume response: the UID and the status are
	// read together, and the UID is what the gateway authorizes on, so it has
	// to be the one the control plane holds now.
...
//
// This hands the test the CA's private key, which is the same trade the MITM
// pool above makes and is acceptable for the same reason: the pool is one blob
// and this runs against a test cluster whose kubeconfig could read it anyway.
// The alternative -- asking ateapi to mint through MintCert -- is not open to a
// test: that RPC authenticates its caller as an atelet over mTLS and derives
// the identity from a worker assignment, so nothing outside a worker node can
// ask for a certificate at all.
...
// atunnel gets from ateapi. It mirrors MintCert
// (cmd/ateapi/internal/actoridentity/actoridentity.go) field for field, because
// a credential that differs from the real one tests a path no actor takes.
