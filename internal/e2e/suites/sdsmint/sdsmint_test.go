...
// its --ttl by an order of magnitude, one signed by the root instead of a
// delegated intermediate -- every one of those carries traffic exactly
// as well as a correct one, and the pod stays Running throughout. Those are the
// assertions below, and no amount of actor traffic makes them.
//
...
const (
	...
	// leafTTL is --ttl on the sdsmint sidecar, and leafSkew is the backdating
...
// certificates, each issued for the name that was asked for, each chaining to
// the MITM CA through a delegated intermediate, each short-lived.
//
// Two of the names are ordinary and three deliberately are not, which is what
// pins --allow-any as deployed: the gateway mints for names nobody enumerated,
// at depths and in TLDs no allowlist pattern would have covered. This test
// used to assert the opposite -- that an SNI outside --allow was refused --
// and its inversion is the whole content of that config change. If the gateway
// is ever given an allowlist again, this test failing is the intended signal,
// not a regression: restore the refusal assertion rather than narrowing the
// names below.
//
...
func TestSdsmintMintsALeafPerSNI(t *testing.T) {
	...
	// reachable under the old "example.com *.example.com", so a half-reverted
	// config -- patterns back, --allow-any gone -- cannot pass this by accident.
	snis := []string{
	...
	for _, sni := range snis {
		...
		if !strings.EqualFold(leaf.Subject.CommonName, sni) {
		...
		// Whatever the root is constrained to, the intermediate has to
		// inherit: a delegated signer that dropped the constraint would be
		// able to sign for names the root itself could not. The installer
		// currently creates the root without --permitted-dns-domain, so this
		// is vacuous on a default install and becomes the real check the
		// moment someone constrains it.
		if len(root.PermittedDNSDomains) > 0 && len(intermediate.PermittedDNSDomains) == 0 {
			t.Errorf("intermediate signing %q carries no dNSName constraint though the root is constrained to %v; the delegated signer can sign for names the root cannot",
				sni, root.PermittedDNSDomains)
		}

		t.Logf("%s: served a %d-cert chain, leaf serial %s valid %v, issued by %q constrained to %v",
			sni, len(chain), serial, validity, intermediate.Subject.CommonName, intermediate.PermittedDNSDomains)
	}
}
...
//
// It is shared for the same reason liveActor is: building and scheduling the
// pod costs more than every handshake in the suite put together, and nothing
// here is pod-specific -- the tests need a probe that can reach the gateway,
// not a fresh one. The probe already supports this, since which credential to
// present is chosen per handshake rather than at startup.
...
//
// Call it through sharedProbe, not directly: the port-forward is torn down with
// the suite rather than with the test that happened to trigger the build, which
// would close it out from under everything that ran after.
func startProbe(t *testing.T, ctx context.Context) *probeClient {
	...
	// Before the pod, not after: the credentials are Secret volumes, so a pod
	// applied first would sit in ContainerCreating on a failed mount and the
	// readiness wait below would report a timeout instead of the real problem.
