...
//
// It has to run in a pod rather than in the test process because the gateway is
// reachable only from inside the cluster and its serving certificate is
// anchored in a ClusterTrustBundle only a pod can project. Everything after the
// front door is the same path a real actor takes: this uses internal/atunnel,
// the same client cmd/ateom-gvisor builds, so a change that breaks actor egress
// breaks this too.
//
// The client certificate is an actor-identity credential the suite mints and
// mounts, standing in for the one atelet fetches from ateapi for a real actor.
// The pod's own podidentity credential is mounted too, but only so a test can
// show the gateway refusing it: it proves "a substrate workload", which is no
// longer enough to open a tunnel.
//
...
//
// A failure comes back with the stage it happened at, not just a message. The
// gateway says no in two very different places -- the front door's mTLS, and
// the CONNECT authorization behind it -- and a test that told them apart by
// matching on error prose would go green the day a wording change made both
// look the same.
package main
...
var (
	...
	// An actor-identity credential, not the pod's own: the gateway trusts only
	// the actor-identity CA on its front door, and its ext_proc sidecar then
	// looks the certified actor up in the control plane. The suite mints this
	// and mounts it as a Secret; a real actor gets the same thing from ateapi
	// by way of atelet.
