...
func gatewayTLSConfig() (*tls.Config, error) {
	const (
		...
		// egress-plugin.ate-system.svc in the leaf, which is the name
...
// serverTLSConfig builds the config extproc serves with. servingBundle is a
...
func serverTLSConfig(servingBundle, clientCAs string, allowedClients []string) (*tls.Config, error) {
	...
	// per request that reads as "the gateway cannot reach extproc".
