...
// — has to add them here, because nothing else in this file knows they are
// secret. Redacting a header that turns out to be harmless costs one log
// field, while missing one costs a leaked credential, so err towards adding.
